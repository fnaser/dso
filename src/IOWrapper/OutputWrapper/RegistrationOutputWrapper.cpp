...
dso::IOWrap::RegistrationOutputWrapper::RegistrationOutputWrapper(int w, int h)
{
    ...
    this->roi_pts_.push_back(
            Eigen::Vector3d(-0.5467, -0.0263, 1.1294));
    this->roi_pts_.push_back(
            Eigen::Vector3d(-0.6352, -0.0863, 1.0748));
    this->roi_pts_.push_back(
            Eigen::Vector3d(-0.5596, -0.0249, 1.0518));
    this->roi_pts_.push_back(
            Eigen::Vector3d(-0.6046, -0.0019, 0.9655));
    this->roi_pts_.push_back(
            Eigen::Vector3d(-0.6801, -0.0633, 0.9885));
...
dso::IOWrap::RegistrationOutputWrapper::~RegistrationOutputWrapper()
{
    ...
    this->vectorToFile(point_cloud_);
}
...
void dso::IOWrap::RegistrationOutputWrapper::vectorToFile(
        std::vector<Eigen::Matrix<Sophus::SE3Group<double>::Scalar, 3, 1>> vec) {
    std::cout << "vec to file" << std::endl;

    std::ofstream tmp_file;
    tmp_file.open("/home/fnaser/example.csv"); //TODO param
    ...
    if (tmp_file.is_open()) {
        ...
        for(int i=0; i < vec.size(); i++) {
            x.append(std::to_string(vec[i][0]) + ",");
            y.append(std::to_string(vec[i][1]) + ",");
            z.append(std::to_string(vec[i][2]) + ",");
        }
...
void dso::IOWrap::RegistrationOutputWrapper::showImgs() {
    ...
    this->drawFilledCircle(img_start, img_pts);
    ...
    for (int i=start_idx_+1; i < start_idx_+seq_length_; i++) {
        ...
        this->storeImgs(tmp_img, i); //TODO param
    }
}
